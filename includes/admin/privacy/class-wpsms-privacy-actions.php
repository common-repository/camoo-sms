...
class Privacy_Actions
{
    ...
    public function __construct()
    {
        ...
        add_action('admin_post_save_' . $this->metabox, [$this, 'on_save_changes_metabox']);
        ...
        if (isset($_POST['wp_camoo_sms_nonce_privacy']) and isset($_POST['submit']) and (isset($_POST['mobile-number-delete']) || isset($_POST['mobile-number-export']))) {
            if (wp_verify_nonce($_POST['wp_camoo_sms_nonce_privacy'], 'wp_camoo_sms_nonce_privacy')) {
                ...
                if ($_POST['submit'] == __('Delete')) {
                    wp_redirect(admin_url(add_query_arg(['page' => 'wp-camoo-sms-subscribers-privacy', 'delete_mobile' => $mobile], 'admin.php')));
                    exit;
                }
            }
        }
    ...
    {
        if (empty($mobile)) {
            wp_redirect(admin_url(add_query_arg(['page' => 'wp-camoo-sms-subscribers-privacy', 'error' => 'empty_number'], 'admin.php')));
            exit;
        }
    }
    ...
    public function check_user_exist_mobile($mobile)
    {
         ...
         */
        $get_user = get_users(['meta_key' => 'mobile', 'meta_value' => $mobile, 'meta_compare' => '=', 'fields' => 'all_with_meta']);
        if (count($get_user) > 0) {
            foreach ($get_user as $user) {
                //Get User Data
                $result[] = ['FullName' => $user->first_name . ' ' . $user->last_name, 'Mobile' => $user->mobile, 'RegisterDate' => $user->user_registered];
        ...
        if (count($get_user) > 0) {
            foreach ($get_user as $user) {
                //Get User Data
                $result[] = ['FullName' => $user['name'], 'Mobile' => $user['mobile'], 'RegisterDate' => $user['date']];

                //Remove User data if Delete Request
                if ($_POST['submit'] == __('Delete')) {
                    $wpdb->delete($wpdb->prefix . 'camoo_sms_subscribes', ['ID' => $user['ID']], ['%d']);
                }
            }
        }

        if (empty($result)) {
            wp_redirect(admin_url(add_query_arg(['page' => 'wp-camoo-sms-subscribers-privacy', 'error' => 'not_found'], 'admin.php')));
            exit;
        }
